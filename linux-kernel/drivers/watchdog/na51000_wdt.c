...
static inline void nvt_wdt_start(void)
{
	...
	if ((WDT_OPEN == false) && (wdt_state == NVTWDT_EXIT)) {
		pr_err("WDT is not opened!\r\n");
		return;
	}

	/*Enable daemon if ioctrl open path*/
	if (wdt_state == NVTWDT_EXITCLR)
...
static inline void nvt_wdt_reload(void)
{
	...
	if (WDT_OPEN == false) {
		pr_err("Reload check WDT is not opened!\r\n");
		return;
	}
