...
{
	/* Add for rts gpio */
	if (p->rts_gpio_enable) {
		int ret;

		ret = p->rts_active_high ? 0 : 1;
		if (gpio_get_value(p->rts_gpio) != ret) {
			if (p->delay_rts_after_send > 0)
				mdelay(p->delay_rts_after_send);
	...
	/* Add for rts gpio */
	if (p->rts_gpio_enable && !(p->rs485_rx_during_tx)) {
		/*
...
static void serial8250_start_tx(struct uart_port *port)
{
	...
	if (!(up->ier & UART_IER_THRI)) {
		...
		/* Add for rts gpio */
		if (up->rts_gpio_enable) {
			int ret;

			ret = up->rts_active_high ? 1 : 0;
			if (gpio_get_value(up->rts_gpio) != ret) {
				gpio_set_value(up->rts_gpio, ret);
				if (up->delay_rts_before_send > 0)
					mdelay(up->delay_rts_before_send);
			}

			if (!(up->rs485_rx_during_tx)) {
				     ...
				     unsigned int count)
{
	...
	if (up->canary && (up->canary != serial_port_in(port, UART_SCR))) {
		...
		if (port->hw_flowctrl)
			serial_port_out(port, UART_MCR, UART_MCR_DTR | UART_MCR_AFC);
		else
...
int serial8250_register_8250_port(struct uart_8250_port *up)
{
	...
	if (uart && uart->port.type != PORT_8250_CIR) {
		...
		/* Add for rts gpio */
		if (up->rts_gpio_enable) {
			uart->rts_gpio_enable= up->rts_gpio_enable;
			uart->rts_gpio = up->rts_gpio;
			uart->rts_default_value = up->rts_default_value;
			uart->rts_active_high = up->rts_active_high;
			uart->delay_rts_before_send = up->delay_rts_before_send;
			uart->delay_rts_after_send = up->delay_rts_after_send;
			uart->rs485_rx_during_tx = up->rs485_rx_during_tx;

			ret = gpio_request(uart->rts_gpio, "serial_rts");
			if (ret < 0)
				pr_err("rts gpio request failed\n");

			ret = gpio_direction_output(uart->rts_gpio, uart->rts_default_value);
			if (ret < 0)
				pr_err("rts gpio dir output failed\n");
		}
