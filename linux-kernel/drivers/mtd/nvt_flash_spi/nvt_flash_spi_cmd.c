...
#define HVY_LOAD_BURST_128          (0x7F<<8)

#define DDR_INREG32(x)                      (*((volatile UINT32*)(NVT_PERIPHERAL_VIRT_BASE + x)))
#define DDR_OUTREG32(x, y)                  (*((volatile UINT32*)(NVT_PERIPHERAL_VIRT_BASE + x)) = (y))
#define DDR_SETREG32(x, y)                  DDR_OUTREG32((x), DDR_INREG32(x) | (y))
#define DDR_CLRREG32(x, y)                  DDR_OUTREG32((x), DDR_INREG32(x) & ~(y))
...
u16 hwcopy_chksum(u32 addr, u32 length)
{
	u32  reg;
	u16  checksum;
	if (addr & 0x03) {
                pr_err("addr need word align\r\n");
		return -1;
        }
        if (length & 0x03) {
                pr_err("length need word align\r\n");
		return -1;
        }
	DDR_OUTREG32(0x8060, (HVY_LOAD_SKIP_COMPARE | HVY_LOAD_READ_CHKSUM | HVY_LOAD_BURST_128));
	DDR_OUTREG32(0x8064, virt_to_phys((u32*)addr));
	DDR_OUTREG32(0x8068, length);
	DDR_OUTREG32(0x8060, (HVY_LOAD_SKIP_COMPARE | HVY_LOAD_READ_CHKSUM | HVY_LOAD_BURST_128 | 0x1));

	reg = DDR_INREG32(0x8060);

	while((reg & 0x00000001) == 0x00000001) {
		reg = DDR_INREG32(0x8060);
	}
	checksum = ((DDR_INREG32(0x806C) & 0xFFFF0000) >> 16) + (((1+length)*length)>>1);
	...
	do
	{
		...
		if (info->flash_info->plane_flags && (row_addr & (0x1 << 6))) {
			...
			if (col_addr == info->flash_info->page_size) {
				...
				dma_flushwritecache((uint32_t*)(pul_buf), info->flash_info->oob_size);
			} else {
				nand_host_setup_transfer(info, &trans_param, 1, info->flash_info->page_size + info->flash_info->oob_size, _SINGLE_PAGE);

				dma_flushwritecache((uint32_t*)(pul_buf), (info->flash_info->page_size + info->flash_info->oob_size));
			}
			...
			if (col_addr == info->flash_info->page_size)
				dma_flushreadcache((uint32_t*)(pul_buf), info->flash_info->oob_size);
			else
...
void nand_drvdump(void)
{
	...
	pad_get_drivingsink(PAD_DS_CGPIO9, &driving);
