...
int sdiohost_setpaddriving(struct mmc_nvt_host *host, SDIO_SPEED_MODE mode)
{
	...
	if (host->id == SDIO_HOST_ID_1) {
		pad_set_drivingsink(PAD_DS_CGPIO17, cmd_uidriving);
		pad_set_drivingsink(PAD_DS_CGPIO18, data_uidriving);
		...
		return pad_set_drivingsink(PAD_DS_CGPIO16, clk_uidriving);
	} else if (host->id == SDIO_HOST_ID_2) {
		pad_set_drivingsink(PAD_DS_CGPIO23, cmd_uidriving);
		pad_set_drivingsink(PAD_DS_CGPIO24, data_uidriving);
		pad_set_drivingsink(PAD_DS_CGPIO25, data_uidriving);
		pad_set_drivingsink(PAD_DS_CGPIO26, data_uidriving);
		pad_set_drivingsink(PAD_DS_CGPIO27, data_uidriving);
		return pad_set_drivingsink(PAD_DS_CGPIO22, clk_uidriving);
	} else
		return E_OK;
}
...
void sdiohost_power_down(struct mmc_nvt_host *host)
{
	if (host->id == SDIO_HOST_ID_1) {
		...
		pad_set_pull_updown(PAD_PIN_CGPIO23, PAD_PULLDOWN);
		pad_set_pull_updown(PAD_PIN_CGPIO24, PAD_PULLDOWN);
		pad_set_pull_updown(PAD_PIN_CGPIO25, PAD_PULLDOWN);
		pad_set_pull_updown(PAD_PIN_CGPIO26, PAD_PULLDOWN);
		pad_set_pull_updown(PAD_PIN_CGPIO27, PAD_PULLDOWN);
	
		gpio_direction_output(C_GPIO(22), 0);
		gpio_direction_output(C_GPIO(23), 0);
		gpio_direction_output(C_GPIO(24), 0);
		gpio_direction_output(C_GPIO(25), 0);
		gpio_direction_output(C_GPIO(26), 0);
		gpio_direction_output(C_GPIO(27), 0);
	}
...
void sdiohost_power_up(struct mmc_nvt_host *host)
{
	...
	if (host->id == SDIO_HOST_ID_1) {
		...
		pad_set_pull_updown(PAD_PIN_CGPIO17, PAD_PULLUP);
		...
		gpio_direction_input(C_GPIO(23));
		gpio_direction_input(C_GPIO(24));
		gpio_direction_input(C_GPIO(25));
		gpio_direction_input(C_GPIO(26));
		gpio_direction_input(C_GPIO(27));
		...
		pad_set_pull_updown(PAD_PIN_CGPIO23, PAD_PULLUP);
		pad_set_pull_updown(PAD_PIN_CGPIO24, PAD_PULLUP);
		pad_set_pull_updown(PAD_PIN_CGPIO25, PAD_PULLUP);
		pad_set_pull_updown(PAD_PIN_CGPIO26, PAD_PULLUP);
		pad_set_pull_updown(PAD_PIN_CGPIO27, PAD_PULLUP);
	}
}
...
void sdio_drvdump(void)
{
	...
	if (drvdump_info[1].max_voltage == SDIO_HOST_SETPOWER_VOL_1P8)
		pr_info("SDIO2 max_voltage 1800 mV\n");
	else
		pr_info("SDIO2 max_voltage 3300 mV\n");

	pad_power->pad_power_id = PAD_POWERID_MC1;
	pad_get_power(pad_power);

	if (pad_power->pad_power)
		pr_info("SDIO2 pad_power 1800 mV\n");
	else
		pr_info("SDIO2 pad_power 3300 mV\n");

	pr_info("SDIO2 voltage_switch %d\n", drvdump_info[1].voltage_switch);
	...
	pad_get_drivingsink(PAD_DS_CGPIO23, &driving);
	if (driving & PAD_DRIVINGSINK_5MA)
		pr_info("SDIO2 CMD driving 5 mA\n");
	else if (driving & PAD_DRIVINGSINK_10MA)
		pr_info("SDIO2 CMD driving 10 mA\n");
	else if (driving & PAD_DRIVINGSINK_15MA)
		pr_info("SDIO2 CMD driving 15 mA\n");
	else if (driving & PAD_DRIVINGSINK_20MA)
		pr_info("SDIO2 CMD driving 20 mA\n");
	else if (driving & PAD_DRIVINGSINK_25MA)
		pr_info("SDIO2 CMD driving 25 mA\n");
	else if (driving & PAD_DRIVINGSINK_30MA)
		pr_info("SDIO2 CMD driving 30 mA\n");
	else if (driving & PAD_DRIVINGSINK_35MA)
		pr_info("SDIO2 CMD driving 35 mA\n");
	else if (driving & PAD_DRIVINGSINK_40MA)
		pr_info("SDIO2 CMD driving 40 mA\n");
	else
		pr_info("SDIO2 CMD driving none mA\n");

	pad_get_drivingsink(PAD_DS_CGPIO24, &driving);
	if (driving & PAD_DRIVINGSINK_5MA)
		pr_info("SDIO2 D0 driving 5 mA\n");
	else if (driving & PAD_DRIVINGSINK_10MA)
		pr_info("SDIO2 D0 driving 10 mA\n");
	else if (driving & PAD_DRIVINGSINK_15MA)
		pr_info("SDIO1 D0 driving 15 mA\n");
	else if (driving & PAD_DRIVINGSINK_20MA)
		pr_info("SDIO2 D0 driving 20 mA\n");
	else if (driving & PAD_DRIVINGSINK_25MA)
		pr_info("SDIO2 D0 driving 25 mA\n");
	else if (driving & PAD_DRIVINGSINK_30MA)
		pr_info("SDIO2 D0 driving 30 mA\n");
	else if (driving & PAD_DRIVINGSINK_35MA)
		pr_info("SDIO2 D0 driving 35 mA\n");
	else if (driving & PAD_DRIVINGSINK_40MA)
		pr_info("SDIO2 D0 driving 40 mA\n");
	else
		pr_info("SDIO2 D0 driving none mA\n");

	pad_get_drivingsink(PAD_DS_CGPIO25, &driving);
	if (driving & PAD_DRIVINGSINK_5MA)
		pr_info("SDIO2 D1 driving 5 mA\n");
	else if (driving & PAD_DRIVINGSINK_10MA)
		pr_info("SDIO2 D1 driving 10 mA\n");
	else if (driving & PAD_DRIVINGSINK_15MA)
		pr_info("SDIO2 D1 driving 15 mA\n");
	else if (driving & PAD_DRIVINGSINK_20MA)
		pr_info("SDIO2 D1 driving 20 mA\n");
	else if (driving & PAD_DRIVINGSINK_25MA)
		pr_info("SDIO2 D1 driving 25 mA\n");
	else if (driving & PAD_DRIVINGSINK_30MA)
		pr_info("SDIO2 D1 driving 30 mA\n");
	else if (driving & PAD_DRIVINGSINK_35MA)
		pr_info("SDIO2 D1 driving 35 mA\n");
	else if (driving & PAD_DRIVINGSINK_40MA)
		pr_info("SDIO2 D1 driving 40 mA\n");
	else
		pr_info("SDIO2 D1 driving none mA\n");

	pad_get_drivingsink(PAD_DS_CGPIO26, &driving);
	if (driving & PAD_DRIVINGSINK_5MA)
		pr_info("SDIO2 D2 driving 5 mA\n");
	else if (driving & PAD_DRIVINGSINK_10MA)
		pr_info("SDIO2 D2 driving 10 mA\n");
	else if (driving & PAD_DRIVINGSINK_15MA)
		pr_info("SDIO2 D2 driving 15 mA\n");
	else if (driving & PAD_DRIVINGSINK_20MA)
		pr_info("SDIO2 D2 driving 20 mA\n");
	else if (driving & PAD_DRIVINGSINK_25MA)
		pr_info("SDIO2 D2 driving 25 mA\n");
	else if (driving & PAD_DRIVINGSINK_30MA)
		pr_info("SDIO2 D2 driving 30 mA\n");
	else if (driving & PAD_DRIVINGSINK_35MA)
		pr_info("SDIO2 D2 driving 35 mA\n");
	else if (driving & PAD_DRIVINGSINK_40MA)
		pr_info("SDIO2 D2 driving 40 mA\n");
	else
		pr_info("SDIO2 D2 driving none mA\n");

	pad_get_drivingsink(PAD_DS_CGPIO27, &driving);
	if (driving & PAD_DRIVINGSINK_5MA)
		pr_info("SDIO2 D3 driving 5 mA\n");
	else if (driving & PAD_DRIVINGSINK_10MA)
		pr_info("SDIO2 D3 driving 10 mA\n");
	else if (driving & PAD_DRIVINGSINK_15MA)
		pr_info("SDIO2 D3 driving 15 mA\n");
	else if (driving & PAD_DRIVINGSINK_20MA)
		pr_info("SDIO2 D3 driving 20 mA\n");
	else if (driving & PAD_DRIVINGSINK_25MA)
		pr_info("SDIO2 D3 driving 25 mA\n");
	else if (driving & PAD_DRIVINGSINK_30MA)
		pr_info("SDIO2 D3 driving 30 mA\n");
	else if (driving & PAD_DRIVINGSINK_35MA)
		pr_info("SDIO2 D3 driving 35 mA\n");
	else if (driving & PAD_DRIVINGSINK_40MA)
		pr_info("SDIO2 D3 driving 40 mA\n");
	else
