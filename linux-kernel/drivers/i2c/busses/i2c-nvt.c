...
u8 i2c_nvtim_rst_flg[NVT_I2C_MAX_NR] = {0};
/* module reset for error handle */
static int i2c_nvtim_rst(struct nvtim_i2c_dev *dev)
{
	if (dev->adapter.nr >= NVT_I2C_MAX_NR) {
		dev_err(dev->dev, "%s: nr %d over max\n", __func__, dev->adapter.nr);
		return -1;
	}

	if (i2c_nvtim_rst_flg[dev->adapter.nr]) {
		// module reset
		...
		dev_err(dev->dev, "%s: nr %d staus 0x%.8x\n", __func__, dev->adapter.nr, nvtim_i2c_read_reg(dev, I2C_STS_REG));
	}
...
nvt_i2c_config(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	if (msgs[0].flags == NVT_I2C_SET_CFG_FLG) {
		...
		if (msgs[0].buf[0] == NVT_I2C_SET_RST) {
			if (dev->adapter.nr >= NVT_I2C_MAX_NR) {
				dev_err(dev->dev, "%s: nr %d over max\n", __func__, dev->adapter.nr);
				return;
			}

			i2c_nvtim_rst_flg[dev->adapter.nr] = (msgs[0].buf[1] > 0) ? 1 : 0;
		}
	}
