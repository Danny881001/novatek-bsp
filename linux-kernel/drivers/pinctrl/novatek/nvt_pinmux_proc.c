...
ERR_OUT:
	...
	} else if (!strcmp(argv[0], "writekeyset")) {
	 	UINT32 	key_set = EFUSE_OTP_2ND_KEY_SET_FIELD;
		if (!strcmp(argv[1], "0")) {
			key_set = EFUSE_OTP_1ST_KEY_SET_FIELD;
		} else if(!strcmp(argv[1], "1")) {
		...
		if (efuse_key_get_flag(key_set) == 1) {
			pr_info("Write [%d] key operation but key field already programmed\r\n", (int)(key_set));
		} else {
			if (efuse_write_key(key_set, key_2nd) < 0) {
				pr_info("Write [%d] key operation fail\r\n", (int)(key_set));
			} else {
				pr_info("Write [%d] key operation success\r\n", (int)(key_set));
			}
		}
	}
