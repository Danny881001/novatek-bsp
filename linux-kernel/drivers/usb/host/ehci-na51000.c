...
MODULE_PARM_DESC(usb_eye, "set this field to 20 to enable usb 2.0 highspeed eye test");
...
{
	U2PHY_SETREG(0x35, 0xE2);
	U2PHY_SETREG(0x34, 0xFF);
	U2PHY_SETREG(0x14, 0x2A);
	U2PHY_SETREG(0x14, 0x2B);

	printk("enable usb2.0 port highspeed eye test pattern output\n");
}
...
static void nvtim_init_usbhc(struct platform_device *dev)
{
	...
	/* Configure PHY related settings below */
	{
		...
		INT32 result;
		UINT32 temp;
		UINT8 u2_trim_swctrl=4, u2_trim_sqsel=4;
