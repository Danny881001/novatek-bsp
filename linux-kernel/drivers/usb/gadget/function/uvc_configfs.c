...
static struct configfs_attribute *uvcg_frame_attrs2[] = {
	...
	&uvcg_frame_dw_default_frame_interval.attr,
	&uvcg_frame_dw_frame_interval.attr,
	&uvcg_frame_dw_bytes_perline.attr,
	NULL,
};
...
CONFIGFS_ATTR_STRUCT(uvcg_framebased);
CONFIGFS_ATTR_OPS(uvcg_framebased);

static struct configfs_item_operations uvcg_framebased_item_ops = {
	.show_attribute		= uvcg_framebased_attr_show,
	.store_attribute	= uvcg_framebased_attr_store,
};
...
static ssize_t uvcg_framebased_guid_format_show(struct uvcg_framebased *ch,
							char *page)
{
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
...
static ssize_t uvcg_framebased_guid_format_store(struct uvcg_framebased *ch,
						   const char *page, size_t len)
{
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
...
static struct uvcg_framebased_attribute uvcg_framebased_guid_format =
	__CONFIGFS_ATTR(guidFormat, S_IRUGO | S_IWUSR,
			uvcg_framebased_guid_format_show,
			uvcg_framebased_guid_format_store);


#define UVCG_FRAMEBASED_ATTR_RO(cname, aname, conv)			\
static ssize_t uvcg_framebased_##cname##_show(			\
	struct uvcg_framebased *u, char *page)			\
{									\
	struct f_uvc_opts *opts;					\
	struct config_item *opts_item;					\
	struct mutex *su_mutex = &u->fmt.group.cg_subsys->su_mutex;	\
	...
	return result;							\
									...
									\
static struct uvcg_framebased_attribute				\
	uvcg_framebased_##cname =					\
	__CONFIGFS_ATTR_RO(aname, uvcg_framebased_##cname##_show)
...
{									\
	...
	return result;							\
									...
									\
static ssize_t								\
uvcg_framebased_##cname##_store(struct uvcg_framebased *u,		\
				    const char *page, size_t len)	\
{									\
	struct f_uvc_opts *opts;					\
	struct config_item *opts_item;					\
...
static struct uvcg_framebased_attribute				\
	uvcg_framebased_##cname =					\
	__CONFIGFS_ATTR(aname, S_IRUGO | S_IWUSR,			\
			uvcg_framebased_##cname##_show,		\
			uvcg_framebased_##cname##_store)

#define identity_conv(x) (x)


UVCG_FRAMEBASED_ATTR(b_format_index, bFormatIndex, identity_conv);
UVCG_FRAMEBASED_ATTR(b_bits_per_pixel, bBitsPerPixel, identity_conv);
UVCG_FRAMEBASED_ATTR(b_default_frame_index, bDefaultFrameIndex, identity_conv);
...
static inline ssize_t
uvcg_framebased_bma_controls_show(struct uvcg_framebased *unc, char *page)
{
	return uvcg_format_bma_controls_show(&unc->fmt, page);
}

static inline ssize_t
uvcg_framebased_bma_controls_store(struct uvcg_framebased *ch,
				     const char *page, size_t len)
{
	return uvcg_format_bma_controls_store(&ch->fmt, page, len);
}

static struct uvcg_framebased_attribute uvcg_framebased_bma_controls =
	__CONFIGFS_ATTR(bmaControls, S_IRUGO | S_IWUSR,
			uvcg_framebased_bma_controls_show,
			uvcg_framebased_bma_controls_store);
...
static struct configfs_attribute *uvcg_framebased_attrs[] = {
	&uvcg_framebased_guid_format.attr,
	&uvcg_framebased_b_bits_per_pixel.attr,
	&uvcg_framebased_b_default_frame_index.attr,
	&uvcg_framebased_b_aspect_ratio_x.attr,
	&uvcg_framebased_b_aspect_ratio_y.attr,
	&uvcg_framebased_bm_interface_flags.attr,
	&uvcg_framebased_bma_controls.attr,
	NULL,
};

static struct config_item_type uvcg_framebased_type = {
	.ct_item_ops	= &uvcg_framebased_item_ops,
	...
	case UVCG_FORMAT: {
		...
		if (fmt->type == UVCG_UNCOMPRESSED) {
			...
			*dest += sizeof(m->desc);
			...
			struct uvc_format_framebased *mjp = *dest;
			struct uvcg_framebased *m =
				container_of(fmt, struct uvcg_framebased, fmt);
	...
	case UVCG_FRAME: {
		...
		*dest += sz;
		if (frm->fmt_type == UVCG_UNCOMPRESSED)
			h->bLength = UVC_DT_FRAME_UNCOMPRESSED_SIZE(frm->frame.b_frame_interval_type);
		else if (frm->fmt_type == UVCG_MJPEG)
			h->bLength = UVC_DT_FRAME_MJPEG_SIZE(frm->frame.b_frame_interval_type);
		else if (frm->fmt_type == UVCG_FRAMEBASED)
			h->bLength = UVC_DT_FRAME_FRAMEBASED_SIZE(frm->frame.b_frame_interval_type);
	}
	break;
	}
