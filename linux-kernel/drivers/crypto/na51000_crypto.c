...
static void na51000_crypto_start(struct na51000_crypto_dev *dev)
{
	...
	case NA51000_CRYPTO_MODE_3DES:		/* key => 192 bit, IV => 64 bits, data => 64 bits, DES-EDE3 */
		...
		break;
    case NA51000_CRYPTO_MODE_KEYMANAGER:
        //pr_info("NA51000_CRYPTO_MODE_KEYMANAGER\n");
//        efuse_otp_set_key(1);
        reqctx->mode = NA51000_CRYPTO_MODE_AES_128;
        break;
...
static int na51000_crypto_otp_keymanager_setkey(struct crypto_ablkcipher *tfm, const u8 *key, unsigned int keylen)
{
	int err;
	struct na51000_crypto_ctx *ctx = crypto_ablkcipher_ctx(tfm);

    pr_err("na51000_crypto_otp_keymanager_setkey\n");

	if (unlikely(keylen >= EFUSE_OTP_TOTAL_KEY_SET_FIELD)) {
		/* The requested key size is not supported by HW, do a fallback */
        pr_err("na51000_crypto keymanager keylen = keyset value = (0~3)\n");
		if (ctx->fallback) {
			crypto_ablkcipher_clear_flags(ctx->fallback, CRYPTO_TFM_REQ_MASK);
			crypto_ablkcipher_set_flags(ctx->fallback, crypto_ablkcipher_get_flags(tfm)&CRYPTO_TFM_REQ_MASK);
			err = crypto_ablkcipher_setkey(ctx->fallback, key, keylen);
    ...
    pr_info("na51000_crypto_otp_keymanager_setkey[%d] set\n", keylen);

    efuse_otp_set_key(keylen);
...
static int na51000_crypto_otp_keymanager_encrypt(struct ablkcipher_request *req)
{
	struct crypto_ablkcipher     *tfm    = crypto_ablkcipher_reqtfm(req);
	struct na51000_crypto_ctx    *ctx    = crypto_ablkcipher_ctx(tfm);
	struct na51000_crypto_reqctx *reqctx = ablkcipher_request_ctx(req);

    pr_info("na51000_crypto_aes_ecb_encrypt start nByte = %d, key len = %d\n", req->nbytes, ctx->keylen);
	...
	/* setup operation */
	reqctx->type        = NA51000_CRYPTO_TYPE_ENCRYPT;
	reqctx->mode        = NA51000_CRYPTO_MODE_KEYMANAGER;
    ...
    //pr_info("na51000_crypto_otp_keymanager_encrypt start request \n");
	return na51000_crypto_handle_req(ctx->dev, req);
}

static int na51000_crypto_otp_keymanager_decrypt(struct ablkcipher_request *req)
{
	struct crypto_ablkcipher     *tfm    = crypto_ablkcipher_reqtfm(req);
	struct na51000_crypto_ctx    *ctx    = crypto_ablkcipher_ctx(tfm);
	struct na51000_crypto_reqctx *reqctx = ablkcipher_request_ctx(req);
...
static struct crypto_alg na51000_crypto_algs[] = {
	...
	//==========================================================================
	{
		.cra_name			= "ecb(aes)-efuse",     //km:keymanager
		.cra_driver_name	= "na51000-ecb-aes-efuse",
		.cra_priority		= NA5100_CRYPTO_ALG_PRIORITY,
		...
		.cra_u.ablkcipher = {
			.min_keysize	= 0,
			.max_keysize	= 3,
			.setkey			= na51000_crypto_otp_keymanager_setkey,
			.encrypt		= na51000_crypto_otp_keymanager_encrypt,
			.decrypt		= na51000_crypto_otp_keymanager_decrypt,
		}
	},
	//==========================================================================
	{
